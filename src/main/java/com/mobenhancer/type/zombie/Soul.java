...
public class Soul implements ZombieCustomType {
    ...
    public void onSpawn(Zombie zombie, CreatureSpawnEvent e) {
        ...
        if (zEquip != null) {
            ...
            if (zombie.getType() == EntityType.ZOMBIFIED_PIGLIN)
                zEquip.setHelmet(new ItemStack(Material.PIGLIN_HEAD), false);
    ...
    public void whenAttacked(Zombie zombie, EntityDamageByEntityEvent e) {
        Location dropperSpawn = e.getDamager().getLocation().add(random.nextDouble() * 1.5 - 0.75,
                3 + random.nextInt(4), random.nextDouble() * 1.5 - 0.75);

        if (!dropperSpawn.getWorld().getBlockAt(dropperSpawn).isEmpty())
            dropperSpawn = e.getDamager().getLocation().add(0, 1.5, 0);
    ...
    private static ItemStack splashPot(PotionType potionType) {
        ...
        if (potMeta != null)
            potMeta.setBasePotionType(potionType);
