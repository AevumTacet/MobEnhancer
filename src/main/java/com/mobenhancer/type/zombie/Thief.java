...
public class Thief implements ZombieCustomType {
    ...
    public void onAttack(Zombie zombie, EntityDamageByEntityEvent e) {
        if (!(e.getEntity() instanceof LivingEntity attacked))
            return;

        List<Zombie> nearbyZombies = zombie.getNearbyEntities(3, 1, 3).stream().filter(it -> it instanceof Zombie)
                .map(it -> (Zombie) it).toList();
        if (nearbyZombies.size() > 3)
            return;
